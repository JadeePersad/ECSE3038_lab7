...
def tanklevel():
    ...
    request_dict = request.json 
    try: 
        deployed_tank= Level().load(request_dict)
    except ValidationError as err:
        return(err.messages, 400)
    
    tankinfo= mongo.db.Level.insert_one(deployed_tank)
    tank_id = tankinfo.inserted_id
    tankinuse = mongo.db.Level.find_one({"_id": tank_id})
    tank_json = loads(dumps(tankinuse))
